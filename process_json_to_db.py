...
def save_to_database(result, db_pool):
    ...
    try:
        ...
        logger.error(f"❌ Database error for {result.get('location_name', 'Unknown')}: {e}")
        if conn:
            conn.rollback()
            conn.close()
        return False
...
def process_json_file(filepath, db_pool):
    ...
    try:
        ...
        if 'results' in data:
            ...
            logger.info(f"📄 Processing webapp format JSON with {len(results)} results")
        elif 'locations' in data:
            # Old format
            results = data.get('locations', [])
            logger.info(f"📄 Processing old format JSON with {len(results)} locations")
        else:
            logger.warning(f"⚠️ Unknown JSON format in {filepath}")
            return 0
        ...
        logger.info(f"✅ Processed {success_count}/{len(results)} entries from {os.path.basename(filepath)}")
...
def main():
    """Main function"""
    logger.info("🚀 Starting JSON to Database processor")
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(json_file):
            logger.info(f"📁 Processing specified file: {json_file}")
        ...
        if latest_file:
            ...
            logger.info(f"📅 File modified: {datetime.fromtimestamp(os.path.getmtime(latest_file))}")
