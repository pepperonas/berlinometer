...
async def _perform_scraping(page, url, location_name_from_csv, start_time, retries):
	...
	print("⏰ Extrahiere Öffnungszeiten...")
	opening_hours = await extract_opening_hours(page)
	if opening_hours:
		print(f"✅ Öffnungszeiten gefunden: {opening_hours}")
...
async def extract_opening_hours(page) -> Optional[str]:
	"""
	Extrahiert Öffnungszeiten von Google Maps
	"""
	try:
		...
		hours_selectors = [
			...
			'.OqCZI .G69YSd',  # Google Maps specific
			'[jsaction*="hours"]',
			'.t39EBf .G69YSd'  # Alternative selector
		]

		# Parallel alle Selektoren testen
		tasks = [page.query_selector_all(selector) for selector in hours_selectors]
		all_elements_lists = await asyncio.gather(*tasks, return_exceptions=True)

		# Durch alle gefundenen Elemente iterieren
		for elements_list in all_elements_lists:
			if isinstance(elements_list, list):
				for element in elements_list[:3]:  # Limitiere auf 3 pro Selektor
					try:
						# Prüfe aria-label
						aria_label = await element.get_attribute('aria-label')
						if aria_label:
							cleaned_text = clean_opening_hours_text(aria_label)
							if cleaned_text:
								return cleaned_text
						...
						if text_content:
							cleaned_text = clean_opening_hours_text(text_content)
							if cleaned_text:
								return cleaned_text
					except:
						continue

		# Fallback: Suche im gesamten Seiteninhalt nach Opening Hours Pattern
		page_content = await page.content()
		return extract_opening_hours_from_content(page_content)

	except Exception as e:
		print(f"⚠️ Fehler bei Opening Hours Extraktion: {e}")
		return None


def clean_opening_hours_text(text: str) -> Optional[str]:
	"""
	Bereinigt und validiert Öffnungszeiten-Text
	"""
	if not text or len(text.strip()) < 5:
		return None
	...
	# Filter out JavaScript/HTML contamination
	if any(indicator in text.lower() for indicator in [
		'function()', 'window.', 'var ', 'let ', 'const ',
		'document.', 'return', 'null,', 'APP_OPTIONS',
		'<script', '</script>', '<div', '</div>',
		'getElementById', 'addEventListener', 'setTimeout',
		'[object', 'undefined', 'prototype'
	]):
		return None

	# Filter out very long texts (likely JavaScript)
	if len(text) > 200:
		return None

	# Filter out texts with special characters typical for JavaScript
	if re.search(r'[{}();,"\[\]\\]', text) and not re.search(r'(\d{1,2}):(\d{2})', text):
		return None

	# Deutsche Opening Hours Patterns
	patterns = [
		r'Geöffnet bis (\d{1,2}):(\d{2})',
		r'Geschlossen[\s·]*Öffnet um (\d{1,2}):(\d{2})',
		r'Öffnet um (\d{1,2}):(\d{2})',
		r'Schließt um (\d{1,2}):(\d{2})',
		r'(\d{1,2}):(\d{2})[\s-]+(\d{1,2}):(\d{2})',
		r'Geöffnet[\s·]*(\d{1,2}):(\d{2})[\s-]+(\d{1,2}):(\d{2})',
		r'(\d{1,2})\.(\d{2})[\s-]+(\d{1,2})\.(\d{2})',
		r'Rund um die Uhr geöffnet',
		r'24 Stunden geöffnet',
		r'Geschlossen'
	]
	...
	# Englische Patterns als Fallback
	english_patterns = [
		r'Open until (\d{1,2}):(\d{2})',
		r'Opens at (\d{1,2}):(\d{2})',
		r'Closes at (\d{1,2}):(\d{2})',
		r'Open 24 hours',
		r'Closed'
	]
...
def extract_opening_hours_from_content(content: str) -> Optional[str]:
	"""
	Fallback: Extrahiert Öffnungszeiten aus dem gesamten HTML-Inhalt
	"""
	if not content:
		return None

	# Remove script and style tags entirely to avoid JavaScript contamination
	content = re.sub(r'<script[^>]*>.*?</script>', '', content, flags=re.DOTALL | re.IGNORECASE)
	content = re.sub(r'<style[^>]*>.*?</style>', '', content, flags=re.DOTALL | re.IGNORECASE)

	# Erweiterte Pattern für HTML-Content
	patterns = [
		r'aria-label="([^"]*(?:Geöffnet|Geschlossen|Öffnet|Schließt)[^"]*)"',
		r'>([^<]*(?:Geöffnet|Geschlossen|Öffnet|Schließt)[^<]*)<',
		r'title="([^"]*(?:Geöffnet|Geschlossen|Öffnet|Schließt)[^"]*)"',
		r'data-value="([^"]*(?:Geöffnet|Geschlossen|Öffnet|Schließt)[^"]*)"'
	]

	for pattern in patterns:
		matches = re.findall(pattern, content, re.IGNORECASE)
		for match in matches:
			# Additional filtering before cleaning
			if len(match) > 500 or 'function' in match.lower() or 'window' in match.lower():
				continue

			cleaned = clean_opening_hours_text(match)
			if cleaned:
				return cleaned
