...
async def scrape_live_occupancy(page, url, location_name_from_csv=None, semaphore=None):
    ...
    if semaphore:
        async with semaphore:
            return await _scrape_with_retry(page, url, location_name_from_csv, start_time,
                                            max_retries)
    else:
...
async def process_locations(locations):
    ...
    individual_processing_times = [r.get('statistics', {}).get('processing_time_seconds', 0) for r
                                   in results]
    ...
    final_data = {
        'metadata': {
            ...
            'success_rate_percent': round((len(successful_requests) / total) * 100,
                                          1) if total > 0 else 0,
            ...
            'locations_per_minute': round((total / (total_execution_time / 60)),
                                          1) if total_execution_time > 0 else 0,
    ...
    print(
        f"📊 Erfolgsrate: {len(successful_requests)}/{total} ({(len(successful_requests) / total) * 100:.1f}%)")
