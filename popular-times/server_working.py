...
# Integrated scraping function - Using the WORKING version from local script
import asyncio
...
async def scrape_live_occupancy(url):
    """
    Scrapt Live-Auslastungsdaten von Google Maps - WORKING VERSION
    """
    async with async_playwright() as p:
        ...
        try:
            ...
            # Alle Selektoren parallel abfragen - EXACTLY like working script
            tasks = []
...
async def process_urls_stream(urls):
    ...
    for i, url in enumerate(urls):
        try:
            ...
            # Add delay between requests to be respectful - same as working script
            if i < total - 1:
...
def scrape_locations():
    ...
    try:
        ...
        logger.info(f"Starting scraping for {len(valid_urls)} URLs using WORKING method")
...
if __name__ == '__main__':
    logger.info("🚀 Starting Popular Times Scraper Server (Working Edition) on port 5044...")
