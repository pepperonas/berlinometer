import { useEffect } from 'react'
import { useTheme } from '../../contexts/ThemeContext'
import './SideDrawer.css'

/**
 * SideDrawer Component
 * Slide-in navigation drawer with user menu
 *
 * @param {boolean} isOpen - Whether the drawer is open
 * @param {Function} onClose - Callback to close the drawer
 * @param {Object} user - User object with name and email
 * @param {Function} onNavigate - Callback for navigation items (receives item id)
 * @param {Function} onLogout - Callback for logout
 */
function SideDrawer({ isOpen, onClose, user, onNavigate, onLogout }) {
  const { theme, switchTheme, availableThemes } = useTheme()

  // Close drawer on ESC key
  useEffect(() => {
    const handleEscape = (e) => {
      if (e.key === 'Escape' && isOpen) {
        onClose()
      }
    }

    if (isOpen) {
      document.addEventListener('keydown', handleEscape)
      // Prevent body scroll when drawer is open
      document.body.style.overflow = 'hidden'
    }

    return () => {
      document.removeEventListener('keydown', handleEscape)
      document.body.style.overflow = 'unset'
    }
  }, [isOpen, onClose])

  if (!isOpen) return null

  const handleNavigationClick = (itemId) => {
    onNavigate(itemId)
    onClose()
  }

  const handleLogout = () => {
    onLogout()
    onClose()
  }

  return (
    <>
      {/* Backdrop */}
      <div
        className="side-drawer__backdrop"
        onClick={onClose}
        aria-hidden="true"
      />

      {/* Drawer */}
      <nav className="side-drawer" role="navigation" aria-label="Hauptnavigation">
        <div className="side-drawer__content">
          {/* User Header */}
          <div className="side-drawer__header">
            {user && (
              <>
                <div className="side-drawer__avatar">
                  {user.username?.charAt(0)?.toUpperCase() || user.email?.charAt(0)?.toUpperCase() || '?'}
                </div>
                <div className="side-drawer__user-info">
                  <div className="side-drawer__user-name">{user.username || user.email || 'User'}</div>
                  <div className="side-drawer__user-email">{user.email}</div>
                </div>
              </>
            )}
          </div>

          {/* Navigation Items */}
          <div className="side-drawer__nav">
            <button
              className="side-drawer__nav-item"
              onClick={() => handleNavigationClick('locations')}
              type="button"
            >
              <span className="side-drawer__nav-icon">📍</span>
              <span className="side-drawer__nav-label">Meine Locations</span>
            </button>

            <button
              className="side-drawer__nav-item"
              onClick={() => handleNavigationClick('profile')}
              type="button"
            >
              <span className="side-drawer__nav-icon">👤</span>
              <span className="side-drawer__nav-label">Profil</span>
            </button>

            <div className="side-drawer__divider"></div>

            {/* Theme Selector */}
            <div className="side-drawer__theme-section">
              <div className="side-drawer__section-title">Design</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {Object.keys(availableThemes || {}).map((themeKey) => (
                  <button
                    key={themeKey}
                    className="side-drawer__nav-item"
                    onClick={() => switchTheme(themeKey)}
                    style={{
                      backgroundColor: theme === themeKey ? 'rgba(104, 141, 177, 0.2)' : 'transparent',
                      borderLeft: theme === themeKey ? '3px solid var(--accent-blue)' : '3px solid transparent'
                    }}
                    type="button"
                  >
                    <span className="side-drawer__nav-icon">
                      {themeKey === 'dark' && '🌙'}
                      {themeKey === 'light' && '☀️'}
                    </span>
                    <span className="side-drawer__nav-label">
                      {themeKey === 'dark' && 'Dark'}
                      {themeKey === 'light' && 'Light'}
                    </span>
                  </button>
                ))}
              </div>
            </div>

            <div className="side-drawer__divider"></div>

            <button
              className="side-drawer__nav-item"
              onClick={() => handleNavigationClick('about')}
              type="button"
            >
              <span className="side-drawer__nav-icon">ℹ️</span>
              <span className="side-drawer__nav-label">Über Berlinometer</span>
            </button>

            <div className="side-drawer__divider"></div>

            {/* Social Links */}
            <div className="side-drawer__social-section">
              <div className="side-drawer__section-title">Connect</div>
              <a
                href="https://www.linkedin.com/in/martin-pfeffer-020831134/"
                target="_blank"
                rel="noopener noreferrer"
                className="side-drawer__nav-item side-drawer__social-link"
              >
                <span className="side-drawer__nav-icon">
                  <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                  </svg>
                </span>
                <span className="side-drawer__nav-label">LinkedIn</span>
                <span className="side-drawer__external-icon">↗</span>
              </a>

              <a
                href="https://github.com/pepperonas"
                target="_blank"
                rel="noopener noreferrer"
                className="side-drawer__nav-item side-drawer__social-link"
              >
                <span className="side-drawer__nav-icon">
                  <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                  </svg>
                </span>
                <span className="side-drawer__nav-label">GitHub</span>
                <span className="side-drawer__external-icon">↗</span>
              </a>
            </div>
          </div>

          {/* Logout Button */}
          <div className="side-drawer__footer">
            <button
              className="side-drawer__logout-button"
              onClick={handleLogout}
              type="button"
            >
              <span className="side-drawer__nav-icon">🚪</span>
              <span className="side-drawer__nav-label">Abmelden</span>
            </button>
          </div>
        </div>
      </nav>
    </>
  )
}

export default SideDrawer
