...
app.get('/api/docs', (req, res) => {
    res.json({
        name: 'Suckyt API',
        version: '2.0.0',
        description: 'YouTube Video Downloader API with authentication',
...
app.post('/api/download', authenticateAPI, rateLimit, async (req, res) => {
    ...
    const outputTemplate = path.join(tempDir, `youtube_video_${timestamp}.%(ext)s`);
    
    // Check if cookie file exists
    const cookieFile = path.join(__dirname, 'youtube-cookies.txt');
    ...
    const tryGalleryDl = () => {
        ...
        const galleryDlArgs = [
            '--dest', tempDir,
            '--filename', `youtube_video_${timestamp}.{extension}`,
            '--no-mtime',
            '--retries', '3'
        ];
        ...
        galleryDl.on('close', (code) => {
            if (code !== 0) {
                ...
                return res.status(500).json({ 
                    ...
                    message: 'Unable to download the video with any available method. This YouTube video may be private or unavailable.',
            ...
            const files = fs.readdirSync(tempDir).filter(file => 
                file.startsWith(`youtube_video_${timestamp}`)
            );
            ...
            const fileName = `youtube_video_${timestamp}.mp4`;
    ...
    if (hasCookies) {
        console.log('Using YouTube cookies for authenticated download');
    }
...
app.listen(PORT, () => {
    console.log(`🚀 Suckyt API server running on port ${PORT}`);
